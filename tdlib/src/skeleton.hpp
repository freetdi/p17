...
public: // types
    ...
    struct sgm{
        ...
        bool operator()(owner_type v) const{ itested();
...
public:
    ...
    template <typename X_t>
    void bag_to_treedec(boost::iterator_range<X_t> const &b, T_t &T, unsigned idx){
        for(auto x : b){
            trace1("insert", x);
            insert(bag(idx, T), x);
        }
    ...
    void do_it(){
        ...
        for(unsigned u = 0; u < max; u++){
            ...
            for(auto bIt : b ){
                unsigned index=_numbering.get_position(bIt);
