...
public:
    typedef CFGT_t<G_t> CFG_t; // user config
#if USE_GALA
    typedef detail::gdvv OverlayG_t;
#else
    typedef detail::baluo OverlayG_t;
#endif
    typedef overlay<G_t, OverlayG_t, boost::iterator_property_map<BOOL*,
				boost::typed_identity_property_map<vertex_descriptor> > > default_overlay_type;
...
protected: // recursion.
    generic_elimination_search_base(generic_elimination_search_base& o);
	 ...
	 void found_ub(unsigned u){
		 if(u < _global_ub){
//			 CFG_t::message(0, "found new upper %d\n", u);
			 CFG_t::commit_ub(u);
			 _global_ub = u;
		 }else{
		 }
	 }
	 void found_lb(unsigned){
		 incomplete();
	 }
...
public:
    ...
    unsigned global_lower_bound_bagsize(){ return _global_lb; }
    unsigned global_upper_bound_bagsize(){ return _global_ub; }

	 // range_t bagsize_range(){ ...}
