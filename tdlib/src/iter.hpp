        ...
        component_iter& operator++()
        {
            ...
            while(true){
                ...
                if(candi.first==candi.second){
                    ...
                    unsigned pos=GETPOS(*candi.first, _cs._g);
                    if (!_cs._visited[pos]){
                        // trace1("visiting", pos);
                        _cs._visited.visit(pos);
                        break;
            ...
            assert(is_end() || _cs._visited[GETPOS(**this, _cs._g)]);
...
//        return &*_range.first < &*other._range.first;
    ...
    {
        BOOST_AUTO(p, GETPOS(*_range.first, _g));
        (void)p;
...
//    }
   ...
   void maybe_push_back(T x)
   {
       ...
       while(true){
           BOOST_AUTO(pos, GETPOS(*x.first, _g));
           if(visited(pos)){
               ++x.first;
           }else{
    ...
    private:
        ...
        void skip_and_visit()
        {
            ...
            while(_onion._q.begin() != _onion._layerend){
                ...
                if(_onion._q.begin() == _onion._layerend){
                    // don't set visited!
                    return;
                }else if (front_range().first == front_range().second ){
                    _onion._q.pop_front();
...
//            trace1("++ in layer", count_range(front_range()));
    ...
    private:
        ...
        void next_nonvisited_or_end()
        {
            while(front_range().first!=front_range().second){
                vertex_descriptor v=*front_range().first;
                BOOST_AUTO(pos, GETPOS(v, _onion._g));
                if(visited(pos)){
                }else{
                    visit(pos);
