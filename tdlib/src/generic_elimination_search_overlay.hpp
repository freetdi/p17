...
void delete_top_edges(G_t &G, VD_t v, unsigned howmany){ itested();
...
//    using concat_iterator=treedec::draft::concat_iterator<i1, i2>;

    struct active_filter{
        active_filter(ACTMAP const & v) : _v(v) {}
        bool operator()(vertex_descriptor v) const{ itested();
            return _v[v];
        }
        ACTMAP const& _v;
    };
...
//    }
	...
	assert(_changes_container.size()==1);
        ...
        for(; vs.first!=vs.second; ++vs.first){ itested();
...
public:
    ...
    adjacent_vertices(vertex_descriptor v) const{
        assert(v<boost::num_vertices(_g));
        assert(v<boost::num_vertices(_og));
...
private:
    ...
    std::vector<long> _elim_stack; // need union {descr, degree}
...
{ untested();
    ...
    while(!_changes_container.top().empty()){ untested();
...
void treedec::gen_search::overlay<A, B, C>::reset_neigh(vertex_descriptor v)
{
    ...
    for(; p2.first!=p2.second; ++p2.first){ itested();
        assert(_elim_stack.back()+1>=0);
        auto olddegree=boost::out_degree(*p2.first, _og);
        detail::delete_top_edges(_og, *p2.first, _elim_stack.back() + 1);
        assert(boost::out_degree(*p2.first, _og) == olddegree - _elim_stack.back() - 1);
        _degree[*p2.first] -= _elim_stack.back();
        _elim_stack.pop_back();
        ...
        typename overlay<A, B, C>::vertex_descriptor elim_vertex)
{
    ...
    _elim_stack.resize(_elim_stack.size() + _degree[elim_vertex] + 1);
    auto seek=_elim_stack.rbegin();
    *seek=elim_vertex;
    ...
    for(; p3.first!=p3.second; ++p3.first){ itested();
...
typename overlay<A, B, C>::vertex_descriptor overlay<A, B, C>::undo_eliminate()
{
    ...
    auto elim_vertex=_elim_stack.back();
