...
// be more selective about absorbable, if neighbourhoods are big
#ifdef NODYNAMIC
#define FN_THRESH 0
#else
#define FN_THRESH 0
...
public: // types
//	TA_GRAPH<T> _g;
	//typedef typename someset<G>::type T; // incomplete.
	typedef CFGT<G> CFG;
	...
	typedef unsigned vertex_t; // T::value_type?
	class BLOCK { //
	public:
		T component;
	private:
		...
		Tns _delta;
	public:
		// T& neighbours_hack(){ return _neighbors; }
		Tns& delta_hack(){ return _delta; }
...
private:
	size_t n() const{ itested();
	  	return _g.num_vertices();
	}
//	NODE* newNode(int v, BLOCK* block, NODE* left, NODE* right);
	void allocate();
	void deallocate();
	void clear();
	void clear_tries();
...
private: // graph access. ideally forward to tdlib
	...
	std::pair<bool, vertex_t> is_saturated(T const& vertices,
...
private: // rewrite
	...
	void extendByNew(
		    typename trie_t::const_iterator	const& node, // ??
			 vertex_t v,
...
//   trie_allocator _shared_trie_area;
...
// c: a union of compatible fscs.
// cand: will be the open neighborhood of the FSCS
EXTA_t
...
//
// outcome
// - feasible (true is returned)
//   c is replaced by sat(c+v)
//   cand: the onb of the resulting FSCS
//   delta: sat \ c
// - not feasible (false returned)
EXTA_t
template<class D>
inline bool exact_ta<EXTA_a>::resaturate_connected(
		T& c, T const& onb, vertex_t v, T& newonb, D& delta)
{ untested();
	incomplete();
	/// v is not an absorbable
...
//  T onb; //  =top_block().neighbours_hack();
  // tassert(onb == _g.openNeighbor(component)); not trimmed.
...
#if 0
    bool x=resaturate_connected(c, b->neighbours(), v, nn, _delta);
#else
...
{
	//	comm_c=NULL;
	// c is the union of compatible FSCS
	// neighb is the neigbours
	// v is a common neighbor of these FSCS
	// comm_c is the common neighbours
	// node is pointing to another FSCS.
	tassert(node->block);
	...
	if (cbset::size(un) > _bag_size) {
		...
		if(!comm_c){
		}else if(comm_c->size()<6){ untested();
			comm_c = NULL;
		}else if(node->block->neighbours().size()<6){ untested();
			comm_c = NULL;
		}
		...
		auto commons(make_lazy_intersection(*cc, node->block->neighbours()));
		auto abs=is_saturated(uc, un, v);
		// firstneigh is absorbable => abs.second==firstneigh
		//
		// in that case abs.second!=v for most v.
		...
		if(abs.first){
			...
			}else if(comm_c==&_singleton){ untested();
				try_extend_by_vertex(uc, un, v, comm_c);
			}else if(comm_c){ untested();
				// pass on neigh info
				commons.begin(); // yuck
				try_extend_by_vertex(uc, un, v, &commons);
			}
			...
			stcnt(ST_rec);
			{
				if(!comm_c){
					...
					try_extend_union(node.back(), uc, un, v, &_singleton);
				}else if(comm_c){ untested();
					// compute intersection. pass it down.
					commons.begin(); // yuck
					T const& nnn_(commons);
					assert(cbset::isSubset(nnn_, un));
					assert(cbset::contains(nnn_, v));
					if(nnn_.size()==1){ untested();
						try_extend_union(node.back(), uc, un, v, &_singleton);
					}else{ untested();
						try_extend_union(node.back(), uc, un, v, &nnn_);
					}
				}
			}
			...
			// not saturated.
			// v is absorbable.
			// make union, add v, resat and (maybe) register
			//
			// maybe do once for the absorbable set, and once for each non-absorbable

			// only necessary if v is the first in the intersection of the involved FSCS
			//if(v==firstneigh){ untested();
				// not necessary if not all commons are absorbable.
				// how to check that?
				if(!comm_c){
					try_extend_by_vertex(uc, un, v);
				}else{ untested();
					//commons.begin(); // yuck
					if(v==*commons.begin()){ untested();
						// need this only once. as all commons are absorbable.
						try_extend_by_vertex(uc, un, v, &commons);
					}else{ untested();
						stcnt(ST_by2);
					}
			...
			// i.e. found an absorbable. but it's not v.
		}
	}
}
		...
		)
{
	...
	for( ; x.first!=trie_t::end(); x.first.inc(&cond)){
		...
		//				auto intbegin=T::intersection_iterator(c, node->block->component);
		//				vertex_t front=*intbegin;

		try_combine_new(x.first, v, uc, un, nnn);
